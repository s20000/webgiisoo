...
public class upgrade extends Model {
	...
	public void edit() {
		...
		if (method.isPost()) {
			...
			SystemConfig.setConfig(conf.getString("node")
					+ ".upgrade.framework.enabled", "on".equals(this
					.getString("upgrade_enabled")) ? "true" : "false");

			SystemConfig.setConfig(conf.getString("node")
					+ ".upgrade.framework.url", this.getString("upgrade_url"));

			SystemConfig.setConfig(conf.getString("node")
					+ ".upgrade.framework.modules",
					this.getString("upgrade_modules"));
		...
		this.set(
				"upgrade_enabled",
				SystemConfig.s(conf.getString("node")
						+ ".upgrade.framework.enabled", "false"));
		this.set(
				"upgrade_url",
				SystemConfig.s(conf.getString("node")
						+ ".upgrade.framework.url", ""));
		this.set(
				"upgrade_modules",
				SystemConfig.s(conf.getString("node")
						+ ".upgrade.framework.modules", ""));
	...
	public void get() {
		...
		String name = new StringBuilder("webgiisoo_")
				.append(Module.load("default").getVersion()).append("_")
				.append(Module.load("default").getBuild()).append(".zip")
				.toString();
		String modules = this.getString("modules");
		...
		try {
			...
			if (!file.exists()) {
				...
				if (!X.isEmpty(modules)) {
					...
					for (String s : ss) {
						Module m = Module.load(s);
						if (m != null) {
							getmodules.append(";").append(s).append(":")
									.append(m.getVersion()).append(".")
									.append(m.getBuild());
			...
			UpgradeLog.insert(V
					.create("modules", getmodules.toString())
					.set("host", ip)
					.set("remote", remote)
					.set("_release",
							SystemConfig.s(conf.getString("node") + ".release",
									""))
					.set("build",
							SystemConfig.s(conf.getString("node") + ".build",
									"")).set("flag", UpgradeLog.FLAG_SERVER));
	...
	public void history() {
		...
		Beans<UpgradeLog> bs = UpgradeLog.load(s, n);
		if (bs != null) {
