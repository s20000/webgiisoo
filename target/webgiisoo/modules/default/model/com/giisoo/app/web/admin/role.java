...
public class role extends Model {
	 ...
	 * Adds the.
	 */
	@Path(path = "add", login = true, access = "access.admin")
	public void add() {
		if (method.isPost()) {

			String name = this.getString("name");
			String memo = this.getString("memo");
			int id = Role.create(name, memo);
			if (id > 0) {
	 ...
	 * Edits the.
	 */
	@Path(path = "edit", login = true, access = "access.admin")
	public void edit() {
		if (method.isPost()) {
			...
			if (r != null) {
				if (r.update(V.create("name", name).set("memo",
	 ...
	 * Delete.
	 */
	@Path(path = "delete", login = true, access = "access.admin")
	public void delete() {
		String ids = this.getString("id");
		int updated = 0;
		if (ids != null) {
			...
			for (String s : ss) {
				...
				if (i > 0) {
					updated += i;
					OpLog.info(Role.class, "delete", r.getName(),null,
