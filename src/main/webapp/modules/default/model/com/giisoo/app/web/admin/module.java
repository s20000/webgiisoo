...
public class module extends Model {
	...
	public void add() {
		...
		if (e != null) {
			try {
				 ...
				 */
				String temp = Integer.toString(Bean.millis2Date(System
						.currentTimeMillis()));
				...
				File dest = new File(Model.HOME + "/modules/" + m.getName());
				if (dest.exists()) {
					delete(dest);
				}
	 ...
	 * Disable.
	 */
	@Path(path = "disable", login = true, access = "access.config.admin", log = Model.METHOD_POST
			| Model.METHOD_GET)
	public void disable() {
		String name = this.getString("name");
	 ...
	 * Enable.
	 */
	@Path(path = "enable", login = true, access = "access.config.admin", log = Model.METHOD_POST
			| Model.METHOD_GET)
	public void enable() {
		String name = this.getString("name");
	 ...
	 * Delete.
	 */
	@Path(path = "delete", login = true, access = "access.config.admin", log = Model.METHOD_POST
			| Model.METHOD_GET)
	public void delete() {
		String name = this.getString("name");
		Module m = Module.load(name);
