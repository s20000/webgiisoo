...
public class user extends Model {
    ...
    public void register() {
        ...
        if (method.isPost()) {
            ...
            log.debug("register:" + name + ", " + pwd + ", " + email + ", " + nickname + ", browser=" + browser() + ", isMDC?" + isMDC());
            ...
            String name = this.getString("name");
            if (TConn.ALLOW_USER == null || "*".equals(TConn.ALLOW_USER) || (name != null && name.matches(TConn.ALLOW_USER))) {
                ...
                log.debug("register:" + name + ", " + pwd + ", " + nickname + ", browser=" + browser() + ", isMDC?" + isMDC());
                ...
                log.warn("deny mdc register, name:" + name + ", allowed: " + TConn.ALLOW_USER);
    ...
    public void login() {
        ...
        if (method.isPost()) {
            ...
            if (me != null) {
                ...
                OpLog.warn(User.class, "user.login", lang.get("login.failed") + ":" + name + ", ip:" + this.getRemoteHost(), null);
                ...
                if (u == null) {
                    ...
                    List<User.Lock> list = User.Lock.loadByHost(u.getId(), System.currentTimeMillis() - X.AHOUR, this.getRemoteHost());
                    ...
                    if (list != null && list.size() >= 6) {
                        ...
                        if (list != null && list.size() >= 3) {
                            ...
                            this.put("message", String.format(lang.get("login.name_password.error.times"), list.size()));
                        }
                    }
                }
            ...
            if (TConn.ALLOW_USER == null || "*".equals(TConn.ALLOW_USER) || (name != null && name.matches(TConn.ALLOW_USER))) {
                ...
                try {
                    ...
                    if (u == null) {
                        ...
                        this.set(X.MESSAGE, lang.get("login.name_password.error"));
                    } else if (u.isLocked()) {
                        OpLog.warn(User.class, "login", lang.get("login.locked") + ":" + name + ", ip:" + this.getRemoteHost(), null);
                        ...
                        OpLog.info(User.class, "login", lang.get("login.success") + ":" + name + ", ip:" + this.getRemoteHost(), null);
                    ...
                    log.error(e.getMessage(), e);
                    this.put(X.STATE, X.FAIL);
                    this.put(X.MESSAGE, "name, uid are required, and must match as register");

                    OpLog.warn(User.class, "login", lang.get("login.forbidden") + ":" + name + ", allowed: " + TConn.ALLOW_USER + ", ip:" + this.getRemoteHost(), null);

                }
            } else {
                log.warn("deny the user login:" + name + ", allowed: " + TConn.ALLOW_USER);
                this.put(X.STATE, X.FAIL);
                this.put(X.MESSAGE, "the name is not allowed");

                OpLog.warn(User.class, "login", lang.get("login.forbidden") + ":" + name + ", allowed: " + TConn.ALLOW_USER + ", ip:" + this.getRemoteHost(), null, -1, this.getRemoteHost());
        ...
        if ("true".equals(SystemConfig.s("oauth.enabled", null))) {
            ...
            if (oauth != null && appid != null && key != null) {
                ...
                try {
                    ...
                    url.append(appid).append("/login").append("?data=").append(data);

                    this.redirect(url.toString());

                    return;
                } catch (Exception e) {
                    log.error("oauth=" + oauth + ", appid=" + appid + ", key=" + key, e);
                }
            }
        }
     ...
     * Logout.
     */
    @Path(path = "logout", method = Model.METHOD_GET | Model.METHOD_POST | Model.METHOD_MDC)
    public void logout() {
            ...
            if ("true".equals(SystemConfig.s("oauth.enabled", null))) {
                ...
                if (oauth != null && appid != null && key != null) {
                    ...
                    try {
                        ...
                        url.append(appid).append("/logout").append("?data=").append(data);
    ...
    public void verify() {
        ...
        if ("name".equals(name)) {
            if (User.exists(W.create("name", value).and("deleted", 0, W.OP_EQ).and("locked", 0, W.OP_EQ).and("remote", 0, W.OP_EQ))) {
                ...
                String allow = SystemConfig.s("user.name", "^[a-zA-Z0-9]{4,16}$");
                ...
                if ("login".equals(method)) {
                    if (jo.has("uid") && jo.has("time") && System.currentTimeMillis() - jo.getLong("time") < X.AMINUTE) {
                        ...
                        if (me != null) {
                            ...
                            if ("true".equals(SystemConfig.s("cross.context", X.EMPTY))) {
                                String sessionkey = SystemConfig.s("session.key", "user");
                                JSONObject j1 = new JSONObject();
                                me.toJSON(j1);
                            ...
                            if (oauth != null && appid != null && key != null) {
                                ...
                                try {
                                    ...
                                    url.append(appid).append("/login").append("?data=").append(URLEncoder.encode(Base64.encode(bb), "UTF-8"));
            ...
            if (a != null) {
                ...
                if (data != null) {
                    try {
                        ...
                        this.getSession().set("sso.oauth." + appid, "1").store();

                        // data = URLDecoder.decode(data, "UTF-8");
                        // log.debug("data=" + data);
                        data = new String(DES.decode(Base64.decode(data), a.getKey().getBytes()));
                        ...
                        if (!jo.has("force") || !jo.getBoolean("force")) {
                            ...
                            if (me != null) {
                                ...
                                data = URLEncoder.encode(Base64.encode(DES.encode(jo.toString().getBytes(), a.getKey().getBytes())), "UTF-8");
                        ...
                        this.getSession().set("oauth.callback", url).set("oauth.appkey", a.getKey()).store();
