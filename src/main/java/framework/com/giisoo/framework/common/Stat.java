...
public class Stat extends Bean implements Comparable<Stat> {
	...
	public String toString() {
		return new StringBuilder("Stat(").append(date).append(",")
				.append(Bean.toString(f)).append(",").append(count).append(")")
				.toString();
	}
	...
	public final static List<Stat> load(W w) {
		...
		return Bean.load((String[]) null, w.where(), w.args(),
				w.orderby() == null ? "order by count desc" : w.orderby(), 0,
				-1, Stat.class);
	}
	...
	public static int insertOrUpdate(String module, long date, int uid,
		...
		if (!Bean.exists("date=? and id=?", new Object[] { date, id },
				Stat.class)) {
			V v = V.create("date", date).set("id", id).set("module", module)
					.set("uid", uid).set("count", count)
			 ...
			 */
			return Bean.update(
					"date=? and id=? and count<?",
					new Object[] { date, id, count },
