...
public abstract class MDCServer extends IoHandlerAdapter {
	...
	protected MDCServer(String host, int port) {
		...
		address = (host == null) ? new InetSocketAddress(port)
		...
		synchronized (_conf) {
			 ...
			 */
			if (TConn.pub_key == null || TConn.pri_key == null
					|| "".equals(TConn.pub_key) || "".equals(TConn.pri_key)) {
				/**
				 * print out the old state
				 */
				log.warn("the pub_key or pri_key missed, the old state are pub_key:["
		...
		try {
			...
			while (in.length() > 5) {
				...
				if (len <= 0 || len > MAX_SIZE) {
					log.error("mdcserver.Wrong lendth: " + len + "/" + MAX_SIZE
							+ " - " + session.getRemoteAddress());
				...
				if (in.length() < len) {
					...
					if (d != null) {
						...
						log.error("error to get connection: "
								+ session.getRemoteAddress());
						session.close(true);
					}
				}
