...
public abstract class MDCConnector extends IoHandlerAdapter {
        ...
        try {
            ...
            while (in.length() > 5) {
                ...
                if (len <= 0 || len > MAX_SIZE) {
                    log.error("mdcconnector.Wrong lendth: " + len + "/" + MAX_SIZE + " - " + session.getRemoteAddress());
                    session.close(true);
                    break;
                }
     ...
     * @return the t conn
     */
    public synchronized static TConn connectByTcp(String host, int port, long timeout) {
        ...
        try {
            ...
            log.error("error, [" + host + ":" + port + "], cost: " + t.past() + "ms, timeout=" + timeout, e);
        }
