   ...
   */
	...
	public boolean onResponse(Request in, TConn d) throws Exception {
		...
		if (jo.has(X.STATE) && jo.has(X.CLIENTID) && X.OK_200 == jo.getInt(X.STATE)) {
			...
			if (d.isSupportEncode()) {
				clientid = new String(RSA.decode(Base64.decode(clientid),
						TConn.pri_key));
			}
	...
	public boolean onRequest(Request req, Response resp, TConn d) {
		...
		try {
			 ...
			 */
			d.setCapability(jo.containsKey(X.CAPABILITY) ? jo
					.getInt(X.CAPABILITY) : 0);
			 ...
			 */
			if (TConn.ALLOW_UID == null || "*".equals(TConn.ALLOW_UID)
					|| uid.matches(TConn.ALLOW_UID)) {
				/**
				...
				try {
					...
					while (TConn.exists(clientid)) {
						if (i > 10) {
							log.warn("generate uid exceed 10 times, ip:"
									+ d.getRemoteIp() + ", uid:" + uid);
						}
						clientid = UID.id(d.getRemoteIp(), uid,
								System.currentTimeMillis(), UID.random(10));
						i++;
					}
					...
					if (d.isSupportEncode()) {
						 ...
						 * also
						 */
						clientid = Base64.encode(RSA.encode(
								clientid.getBytes(), key));
					}
					...
					log.error("activate:" + clientid + ", uid:" + uid
							+ ", key:" + key, e);
