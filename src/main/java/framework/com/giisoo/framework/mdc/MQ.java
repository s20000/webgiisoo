...
public final class MQ {
        ...
        if (list != null && list.size() > 0) {
            for (TConn c : list) {
                ...
                if (c1 != null) {
                    ...
                    log.debug("send directly : " + c.getClientId() + "=>" + message + ", binary: " + (bb == null ? 0 : bb.length));
                     ...
                     * send by MQ
                     */
                    count += MQ.send(c.getAddress(), c.getClientId(), ICallback.RESPONSE, message, bb, null, null, null);

                    log.debug("send MQ : " + c.getClientId() + "=>" + message + ", binary: " + (bb == null ? 0 : bb.length));
                } else {
                    log.warn("no mdc or MQ avaliable for uid= " + uid + ", message=" + message);
                }
            }
        } else {
    ...
    public static int sendByClientid(String clientid, String message, byte[] bb) {
        ...
        if (!check()) {
            ...
            if (c != null) {
                ...
                log.debug("send: " + c.getClientId() + "=>" + message + ", binary: " + (bb == null ? 0 : bb.length));
    ...
    public static int response(long uid, String message, byte[] bb) {
        ...
        if (!check()) {
            ...
            if (list != null && list.size() > 0) {
                for (TConn c : list) {
                    count += MQ.response(c.getAddress(), c.getClientId(), ICallback.RESPONSE, message, bb, null, null, null);
                }
            }
        }
        return count;
     ...
     * @return the int
     */
    public static int send(String dest, String to, int command, String message, byte[] bb, String src, String from, String header) {
        if (message == null)
            return -1;
     ...
     * @return the int
     */
    public static int response(String dest, String to, int command, String message, byte[] bb, String src, String from, String header) {
        if (message == null)
            return -1;
     ...
     */
    public static WorkerTask createQueue(final String name, final ICallback cb, final int mode) {
    ...
    private static void process(Request req, ICallback cb) {
        ...
        case ICallback.RESPONSE: {
            ...
            cb.run(command, to, from, src, flag, JSONObject.fromObject(header), JSONObject.fromObject(message), bb);
            break;
        }
        }
    }
