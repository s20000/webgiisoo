...
public class Controller implements ICallback {
	...
	public static void init(Configuration conf, String path) {
		...
		OS = System.getProperty("os.name").toLowerCase() + "_"
	...
	public void run(int command, Object... o) {
		if (o != null && o.length > 0) {
			try {
				...
				final JSONObject header = o.length > 4 ? (JSONObject) o[4]
						: null;
				 ...
				 */
				Controller.dispatch(msg.getString(X.URI), req, resp,
				...
				if (r.size() > 0) {
					...
					MQ.response(src, from, ICallback.RESPONSE, r1.toString(),
							bb, null, null, header2.toString());
				}
			} catch (Exception e) {
	 ...
	 *            the method
	 */
	public static void dispatchWithContextPath(String uri,
			HttpServletRequest req, HttpServletResponse resp, HTTPMethod method) {
	...
	public static void dispatch(String uri, HttpServletRequest req,
			HttpServletResponse resp, Model.HTTPMethod method) {
		...
		if (mo != null) {
			...
			if ("true".equals(SystemConfig.s("access.log.enabled", "false"))) {
				AccessLog.create(
						mo.getRemoteHost(),
						uri,
						V.create("method", method.toString())
								.set("cost", t.past())
								.set("agent", mo.browser())
								.set("sid", mo.sid(false))
								.set("module", mo.module.name)
								.set("handler", mo.getClass().getName())
								.set("header", Bean.toString(mo.getHeaders()))
								.set("query", Bean.toString(mo.getQueries())));
			}
		...
		if (transparenturls == null) {
			...
			if (X.isEmpty(s)) {
				transparenturls = Pattern.compile("^/(css|js|images)/.*$",
			...
			if (m.matches()) {
				...
				if (f == null) {
					...
					try {
						...
						if (f1.exists()
								&& f1.getCanonicalPath().startsWith(home)) {
							/**
		...
		try {
			...
			if (mo != null) {
				...
				if ("true"
						.equals(SystemConfig.s("access.log.enabled", "false"))) {
					AccessLog.create(
							mo.getRemoteHost(),
							uri,
							V.create("method", method.toString())
									...
									.set("handler", mo.getClass().getName())
									.set("header",
											Bean.toString(mo.getHeaders()))
									.set("query",
											Bean.toString(mo.getQueries())));
				}
				...
				if (mo != null) {
					...
					if ("true".equals(SystemConfig.s("access.log.enabled",
							"false"))) {
						AccessLog
								.create(mo.getRemoteHost(),
										uri,
										V.create("method", method.toString())
												.set("cost", t.past())
												.set("agent", mo.browser())
					...
					while (i > 0) {
						...
						if (mo != null) {
							...
							log.info(method + " " + uri + " - " + t.past()
									+ "ms -" + mo.getRemoteHost() + " " + mo);

							if ("true".equals(SystemConfig.s(
