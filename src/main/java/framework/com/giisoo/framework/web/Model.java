...
public class Model {
	 ...
	 *            the method
	 */
	final protected void dispatch(String uri, HttpServletRequest req,
			HttpServletResponse resp, HTTPMethod method) {
		// construct var
		// init
		try {
			...
			if (pathmapping != null) {
				...
				Map<String, PathMapping> methods = pathmapping
							...
							if (path.matches(s)) {
								...
								if (oo != null) {
									...
									if (pp.login()) {
										...
										if (!X.NONE.equals(pp.access())
												&& !login.hasAccess(pp.access()
														.split("\\|"))) {
											/**
											 * no access
											 */
										...
										if ((pp.log() & method.method) > 0) {
											...
											if (context != null) {
												if (context
													...
													jo = new JSONObject();
													if (context
															.containsKey(X.MESSAGE)) {
														jo.put(X.MESSAGE,
																context.get(X.MESSAGE));
														message = jo
																.getString(X.MESSAGE);
													}
													if (context
															.containsKey(X.ERROR)) {
														jo.put(X.ERROR, context
																.get(X.ERROR));
														message = jo
																.getString(X.ERROR);
														error = true;
			...
			case METHOD_GET: {
				...
				if (require == null
						|| !require.login()
						|| (me != null && (require.access() == null
								|| X.NONE.equals(require.access()) || me
									.hasAccess(require.access())))) {
	...
	public String sid(boolean newSession) {
		...
		if (sid2 == null || !sid2.equals(sid)) {
			 ...
			 */
			addCookie("sid", sid, Module._conf.getInt("session.expired",
					(int) (X.AYEAR / 1000)));
		}
		return sid;
	}
	 ...
	 * @return the int
	 */
	final protected int getInt(JSONObject jo, String tag, int minValue,
			String tagInSession) {
		if (jo.has(tag)) {
		...
		try {
			if (this._multipart) {
				...
				if (i != null && i.isFormField()) {
					...
					in.read(bb);
					in.close();
					return new String(bb, "UTF8").replaceAll("<", "&lt;")
							.replaceAll(">", "&gt;");
				}
	...
	final protected String[] getStrings(String name) {
		try {
			if (this._multipart) {
				...
				if (o instanceof FileItem) {
					...
					for (int i = 0; i < ss.length; i++) {
						...
						if (ii.isFormField()) {
							...
							in.read(bb);
							in.close();
							ss[i] = new String(bb, "UTF8").replaceAll("<",
									"&lt;").replaceAll(">", "&gt;");
						}
					}
					return ss;
				...
				if (ss != null && ss.length > 0) {
					for (int i = 0; i < ss.length; i++) {
						ss[i] = ss[i].replaceAll("<", "&lt").replaceAll(">",
								"&gt");
					}
				}
				return ss;
	...
	final protected User getUser() {
		if (login == null) {
			...
			if (login == null
					&& "true".equals(SystemConfig.s("cross.context", "false"))) {
				HttpSession s1 = this.getHttpSession(true);
				if (s1 != null) {
					String ss = (String) s1.getAttribute(SystemConfig.s(
							"session.key", "user"));
					if (ss != null) {
		...
		if ("true".equals(SystemConfig.s("cross.context", "false"))) {
			...
			if (s1 != null) {
				if (u == null) {
					...
					JSONObject jo = new JSONObject();
					u.toJSON(jo);
					s1.setAttribute(SystemConfig.s("session.key", "user"),
							jo.toString());
				}
			}
		}
	...
	final protected Map<String, Object> getFiles() {
		if (uploads == null) {
			...
			ServletContext servletContext = GiisooServlet.config
					.getServletContext();
	...
	final protected boolean show(String viewname, boolean allowOverride) {
		...
		try {
			...
			log.debug("finding template = " + viewname + ", cost: " + t1.past()
					+ "ms");
	...
	final public static String MIME_JSON = "application/json;charset="
			+ ENCODING;
	 ...
	 */
	final public static int copy(InputStream in, OutputStream out)
			throws IOException {
	 ...
	 *             Signals that an I/O exception has occurred.
	 */
	final public static int copy(InputStream in, OutputStream out, long start,
			long end, boolean closeAfterDone) throws IOException {
		try {
			if (in == null || out == null)
				return 0;
	 ...
	 */
	final public static int copy(InputStream in, OutputStream out,
			boolean closeAfterDone) throws IOException {
		try {
			if (in == null || out == null)
				return 0;
	...
	public static class PathMapping {
		 ...
		 */
		public static PathMapping create(Pattern pattern, Path path,
				Method method) {
	...
	public String toString() {
		if (tostring == null) {
			tostring = new StringBuilder(this.getClass().getName()).append("[")
					.append(this.uri).append(", path=").append(this.path)
					.append("]").toString();
		}
		return tostring;
	}
	...
	private boolean validbrowser() {
		...
		if (!X.isEmpty(notsupport)
				&& (agent == null || agent.matches(notsupport))) {
			return false;
		}
	...
	public NameValue[] getQueries() {
		...
		if (e != null) {
			...
			while (e.hasMoreElements()) {
				...
				list.add(NameValue.create(n,
						Bean.toString(req.getParameterValues(n))));
			}
