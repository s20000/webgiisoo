...
public class Module {
	...
	public String toString() {
		return new StringBuilder("Module [").append(id).append(",")
				.append(name).append(",enabled=").append(enabled).append("]")
				.toString();
	}
	 ...
	 */
	public void store() {
		File f = new File(_conf.getString("home") + "/modules/" + name
				+ "/module.ini");
		try {
	...
	public static void init(Configuration conf) {
		try {
				...
				for (File f1 : list) {
					if (f1.isDirectory()) {
						...
						if (m == null) {
							...
							log.error("the [id] duplicated, [" + m.name + ", "
									+ modules.get(m.id).name
									+ "], ignore the [" + m.name + "]");
						} else {
							/**
							 * cache the module
			...
			p.setProperty("file.resource.loader.class",
					"com.giisoo.framework.web.TemplateLoader");
			Velocity.init(p);
	...
	public static Module load(String name) {
		try {
			...
			if (f.exists()) {
				...
				t.pack = p.containsKey("package") ? p.getString("package")
						: null;
				t.lifelistener = p.containsKey("lifelistener") ? p
						.getString("lifelistener") : null;
				t.path = f.getParent();
				if (p.containsKey("id")) {
					t.id = p.getInt("id");
				} else {
					log.error("[id] missed in module:" + t.name
							+ ", ignore the module");
					return null;
				}
				...
				if (p != null) {
					...
					if (!X.isEmpty(access) && !X.NONE.equals(access)) {
						if (access.startsWith("access.")) {
							log.debug("access[" + access + "] at "
									+ c.getCanonicalName() + "." + m.getName());
							...
							log.error("access error! [" + access + "] at "
									+ c.getCanonicalName() + "." + m.getName());
						}
					}
					...
					Model.PathMapping oo = Model.PathMapping.create(
							Pattern.compile(path), p, m);

					/**
					 * set the method mapping info
					 */
					if ((method & Model.METHOD_GET) > 0) {
						Map<String, Model.PathMapping> mm = map
								.get(Model.METHOD_GET);
						if (mm == null) {
	...
	private static String[] source = new String[] { "/model", "/view", "/i18n",
			"/module.ini" };
	...
	public static boolean init(Module m) {
		if (!m.enabled) {
			...
			try {
				 ...
				 */
				m.path = new File(Model.HOME + "/modules/" + m.name)
						.getCanonicalPath();
