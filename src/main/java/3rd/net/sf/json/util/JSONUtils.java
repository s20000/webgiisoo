...
public final class JSONUtils {
    ...
    public static Map getProperties(JSONObject jsonObject) {
        ...
        for (Iterator keys = jsonObject.keys(); keys.hasNext();) {
            ...
            /*
             * String parsedKey = key; if( !JSONUtils.isJavaIdentifier(
             * parsedKey ) ){ parsedKey = JSONUtils.convertToJavaIdentifier( key
             * ); }
             */
     ...
     * Obj must be a non-null String and match
     * "^function[ ]?\\(.*\\)[ ]?\\{.*\\}$"
     */
    ...
    /**
     * Creates a new MorphDynaBean from a JSONObject. The MorphDynaBean will
     * have all the properties of the original JSONObject with the most accurate
     * type. Values of properties are not copied.
     */
     ...
     * right places. A backslash will be inserted within , allowing JSON text to
     * be delivered in HTML. In JSON text, a string cannot contain a control
    ...
    /**
     * Returns true if the input has single-quotes or double-quotes at both
     * sides.
     */
     ...
     *
     * @return true if n is instanceOf Long or the literal value can be
     *         evaluated as a Long.
     */
