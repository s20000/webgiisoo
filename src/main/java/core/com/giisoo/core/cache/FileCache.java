...
public class FileCache {
  ...
  public static void init(Configuration conf) {
    root = conf.getString("home") + "/tmp/_cache/";
    cache_size = conf.getInt("file.cache.size", 1000);
   ...
   */
  public synchronized static boolean set(String id, Object o) {
    try {
      if (o == null) {
        return delete(id);
      } else {
   ...
   * @return true, if successful
   */
  public synchronized static boolean delete(String id) {
  ...
  private static String path(String path) {
    ...
    StringBuilder sb = new StringBuilder(root).append(p1).append("/").append(p2).append("/").append(p3).append("/").append(id);
    return sb.toString();
  }
   ...
   * @return the byte[]
   */
  private static byte[] read(String id) {
    byte[] b = cache.get(id);
   ...
   * @param b the b
   */
  private static void save(String id, byte[] b) {
    cache.put(id, b);
