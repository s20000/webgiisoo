...
public class DefaultCachable implements Cachable {
    ...
    public void cache(String id) throws Exception {
        try {
            Cache.set(new StringBuilder(this.getClass().getName()).append(":").append(id).toString(), this);
        } catch (Exception e) {
            throw e;
        }
    }
    ...
    public static <T extends Cachable> T cache(String id, Class<T> clazz) {
        Cachable o = Cache.get(new StringBuilder(clazz.getName()).append(":").append(id).toString());
