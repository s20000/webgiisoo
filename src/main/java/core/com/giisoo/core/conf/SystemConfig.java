	...
	public synchronized static void setConfig(String name, Object o) {
		...
		if (Bean.exists("name=?", new Object[] { name }, SystemConfig.class)) {
			if (o instanceof Long) {
				Bean.update("name=?", new Object[] { name }, V.create("l", o),
						SystemConfig.class);
			} else if (o instanceof Integer) {
				Bean.update("name=?", new Object[] { name }, V.create("i", o),
						SystemConfig.class);
			} else if (o instanceof Double || o instanceof Float) {
				Bean.update("name=?", new Object[] { name }, V.create("d", o),
						SystemConfig.class);
			} else {
				Bean.update("name=?", new Object[] { name },
						V.create("s", o.toString()), SystemConfig.class);
			}
		} else {
			if (o instanceof Long) {
				Bean.insert(V.create("l", o).set("name", name),
						SystemConfig.class);
			} else if (o instanceof Integer) {
				Bean.insert(V.create("i", o).set("name", name),
						SystemConfig.class);
			} else if (o instanceof Double || o instanceof Float) {
				Bean.insert(V.create("d", o).set("name", name),
						SystemConfig.class);
			} else {
				Bean.insert(V.create("s", o.toString()).set("name", name),
						SystemConfig.class);
			}
		}
	}
