...
public abstract class WorkerTask implements Runnable {
	...
	public String toString() {
		...
		synchronized (t) {
			if (who != null) {
				sb.append(who.getName()).append("(").append(getName())
						.append(")").append(":").append(who.getState());
			} else
			...
			try {
				...
				synchronized (t) {
					...
					if (who != null) {
						...
						log.debug(name + " is running ..., delayed: "
								+ t.past() + "ms, tasks:" + tasksInQueue()
								+ ", active:" + WorkerTask.activeThread()
								+ ", idle:" + WorkerTask.idleThread());
	...
	public static void init(int threadNum, Configuration _conf) {
		executor = new ScheduledThreadPoolExecutor(threadNum,
				new ThreadFactory() {
					...
					public Thread newThread(Runnable r) {
						...
						th.setContextClassLoader(Thread.currentThread()
			...
			public void run() {
				try {
					...
					while (runningQueue.size() > 0) {
						synchronized (lock) {
							try {
								log.info("stoping, size=" + runningQueue.size()
										+ ", running task=" + runningQueue);
