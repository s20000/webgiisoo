...
public class App extends Command {
   ...
   */
        ...
        try {
            ...
            if (jo.has(X.URI) && jo.has(X.SEQ)) {
                ...
                if (d.valid()) {
                    ...
                    if (mquri == null || !uri.matches(mquri) || MQ.send(0, "web", null, jo, bb, TConnCenter.getQueue(), d.getId().toString(), d.getJSON()) <= 0) {
                         ...
                         */
                        Controller.dispatch(jo.getString(X.URI), req, resp, new HTTPMethod(Model.METHOD_MDC));
                        ...
                        log.debug("response: " + r1.toString() + ", bytes:" + (bb == null ? 0 : bb.length));
                    }
                } else {
                    JSONObject r1 = new JSONObject();
    ...
    public boolean onResponse(Request in, TConn d) throws Exception {
        ...
        log.debug("got:" + jsonstr + ", bb:" + (bb == null ? 0 : bb.length) + ", " + d.getRemoteIp());
    ...
    public static void hello(TConn d) {
        ...
        Controller.dispatch(in.getString(X.URI), req, resp, new HTTPMethod(Model.METHOD_MDC));
