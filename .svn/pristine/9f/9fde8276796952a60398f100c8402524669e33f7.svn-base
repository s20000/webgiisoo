...
BeatPicker.prototype = {
	...
	daysFull : [],
	monthsSimple : [ "一", "二", "三", "四", "五", "六", "七", "八", "九", "十", "十一",
			"十二" ],
	...
	// PRIVATE FIELD DONT TRY TO MANIPULATE THESE IF YOU DONT KNOW WHAT ARE YOU
	// DOING
	...
	constructor : function() {
		...
		this._prepareInput();
		// MAIN
		var calendar = this._calendarMainNode = this.pickerNode = this
				._baseDom();
	...
	_setStartDate : function(dateOb) {
		...
		if (typeof dateOb === "string")
			...
			this.startDate = new Date(this.startDate[0], this.startDate[1] - 1,
					this.startDate[2]);
	...
	_prepareInput : function() {
		...
		this.dateInputNode.after(this._inputParentNode = this._domGenerator(
				"div", {
					class : this._addClass(this.className.inputParent,
							this.className._inner.inputContainer)
				}));

		this.dateInputNode.appendTo(this._inputParentNode).addClass(
				this._addClass(this.className.input,
						this.className._inner.inputNode));

		this.selectionRule.range
				&& this.dateInputNode.addClass(
						this._addClass(this.className.inputFrom,
								this.className._inner.dateInputFrom)).after(
						this._dateInputToNode = this._domGenerator("input", {
							type : 'text',
							class : this._addClass(this.className.inputTo,
									this.className._inner.dateInputTo,
									this.className.input,
									this.className._inner.inputNode)
						}, this._inputParentNode));

		this.view.isInputIsReadonly
				&& this.dateInputNode.add(
						this.selectionRule.range ? this._dateInputToNode : {})
						.attr("readonly", true);
	},
	...
	// **********************************************//
	_baseDom : function() {
		return this._domGenerator("div", {
			class : this._addClass(this.className.beatPicker,
					this.className._inner.beatPicker)
		}, this.view.alwaysVisible ? this._inputParentNode : $("body"));
	},
	_createClear : function() {
		var self = this;
		var elem = this._domGenerator("button", {
			class : this._addClass(this.className.clear,
					this.className._inner.clearButton,
					this.className._inner.defaultButton)
		}, null, this.labels.clearButton);
		elem.on("click", function() {
			if (self._selectedDate || self._startRangeSelectedDate
					|| self._endRangeSelectedDate) {
				var options = {
				...
				if (self.selectionRule.range)
					$.extend(options, {
						...
						startDateString : self._startRangeSelectedDate ? self
								._dateFormatting(self._startRangeSelectedDate)
								: null,
						endDate : self._endRangeSelectedDate,
						endDateString : self._endRangeSelectedDate ? self
								._dateFormatting(self._endRangeSelectedDate)
								: null
					});
	...
	_createIcon : function() {
		this.dateInputNode.add(this._dateInputToNode).css("background-image",
				"url('" + this.view.iconImage + "')")
	},
	_createNavBar : function(header) {
		var self = this;
		var navBarParent = this._domGenerator("div", {
			class : this._addClass(this.className.headerNavBarParent,
					this.className._inner.headerNavBar)
		}, header, "", true);

		this._domGenerator(
				"a",
				{
					class : this._addClass(this.className.headerNavBarButton,
							this.className._inner.headerNavBarBtn,
							this.className._inner.headerNavBarBtnNext,
							this.className._inner.defaultButton)
				}, navBarParent, ">").on("click", function() {
			self._increaseUnity(false);
		});

		var currentIndicator = this._currentIndicator = this
				._domGenerator(
						"a",
						{
							class : this
									._addClass(
											this.className.headerNavBarButton,
											this.className._inner.headerNavBarBtn,
											this.className._inner.headerNavBarBtnMonthIndicator,
											this.className._inner.defaultButton)
						}, navBarParent).on('click', function() {
					self._ExpandOrCollapse(true)
				});

		this._currentBriefDateSet(currentIndicator,
				this._briefDateScope.briefDate);
		...
		this._domGenerator(
				"a",
				{
					class : this._addClass(this.className.headerNavBarButton,
							this.className._inner.headerNavBarBtn,
							this.className._inner.headerNavBarBtnPrev,
							this.className._inner.defaultButton)
				}, navBarParent, "<").on("click", function(e) {
			self._increaseUnity(true);
		});

	},
	_createHeader : function() {
		var headerContainer = this._domGenerator("div", {
			"class" : this.className.header
		});
		this.modules.daysOfTheWeek
				&& headerContainer.append(this
						._createDaysOfTheWeek(this.daysSimple));
		return headerContainer;

	},
	_createDaysOfTheWeek : function(days) {
		if (days instanceof Array) {
			var ul = this._weekDayParent = this._domGenerator("ul", {
				class : this.className.weekDaysAliasCellParent + " "
						+ this.className._inner.cellParent
			});
			this.forEach(days, function(index, item) {
				this._domGenerator("li", {
					class : this.className.weekDaysAliasCell + " "
							+ this.className._inner.cell
				}, ul, item);
			}, this);
			return ul;
		}
	...
	_createFooter : function() {
		return this._footerNode = this._domGenerator("div", {
			"class" : this._addClass(this.className.footer,
	...
	_createToday : function(footer) {
		...
		this._domGenerator(
				"a",
				{
					class : this._addClass(this.className.todayButton,
							this.className._inner.todayButton,
							this.className._inner.defaultButton)
				}, footer, this.labels.today).on("click", function(e) {
	...
	_createGotoDate : function(footer) {
		...
		var container = this._domGenerator("div", {
			class : this._addClass(this.className.gotoDateParent,
					this.className._inner.gotoDateContainer)
		}, footer);

		var go = this._domGenerator(
				"input",
				{
					type : "text",
					class : this._addClass(this.className.gotoDateInput,
							this.className._inner.gotoDateInput),
		...
		this._domGenerator(
				...
				{
					type : "button",
					class : this._addClass(this.className.gotoDateButton,
							this.className._inner.gotoDateButton,
							this.className._inner.defaultButton)
				}, container, this.labels.gotoDateButton).on("click",
				function() {
					self._jumpToDateFromInput(go);
				});
	...
	_createDateMatrices : function() {
		var self = this;
		this._notifySubscribers(this.events.change, {
			date : this.currentDate,
			timeStamp : new Date().getTime()
		});
		if (!this._daysBody) {
			var daysBody = this._daysBody = this._domGenerator("div", {
		...
		if (!this._daysCellParent)
			var daysCellParent = this._daysCellParent = this._domGenerator(
					"ul", {
						class : this.className.daysCellParent + " "
								+ this.className._inner.cellParent
					}, this._daysBody);
		if (this.view.display === this._displaySet.days) {
			...
			for (var i = 0; i < 42; i++) {
				...
				li.on("click", function(e) {
					...
					clickBehaviour !== "not-avail"
							&& self._updateDateMatrices(
									clickBehaviour ? 1 : -1, self._monthUnit);
				});
				li.text(dayCheck.day);
				this.selectionRule.single
						&& this._findSelectedDate(this._dateRows[i]
								.data("date"), this._dateRows[i])

			}
			this._markToday(new Date());
			if (this.selectionRule.range)
				this._addClassToRange(this._startRangeSelectedDate,
						this._endRangeSelectedDate);
		} else {
			if (this.view.display === this._displaySet.months) {
				this._createMonths();
				this._currentBriefDateSet(this._currentIndicator,
						this._briefDateScope.yearName);
			}

			else if (this.view.display === this._displaySet.years) {
				this._currentBriefDateSet(this._currentIndicator,
						this._briefDateScope.yearsRange);
				this._createYears();
			}
		}
		return daysBody;
	...
	_createMonths : function() {
		...
		for (var i = 0; i <= 11; i++) {
			var li = this._domGenerator(
					"li",
					{
						class : this._addClass(this.className.monthsCell,
								this.className._inner.cell,
								this.className._inner.cellM)
					}, this._daysCellParent, this.monthsSimple[i]).on("click",
					function() {
	...
	_createYears : function() {
		...
		for (var i = 0; i <= 11; i++) {
			var li = this._domGenerator(
					"li",
					{
						class : this._addClass(this.className.yearsCell,
								this.className._inner.cell,
								this.className._inner.cellY)
	...
	_updateYears : function() {
		...
		for ( var i in this._yearsRows) {
			this._yearsRows[i].data("date", this._setAppropriateDate(years[i],
					false));
	...
	_eventAndPositioning : function() {
		...
		this.selectionRule.range
				&& (elem = this.dateInputNode.add(this._dateInputToNode));
		elem.on(this.view.showOn, function(e) {
			e.stopPropagation();
			if (self._isHide || self.selectionRule.range) {
				var filter = $.grep(window.beatPickerList,
						function(item, index) {
	...
	_positionCalculation : function(currentTarget, pos) {
		...
		var calculationMap = {
			...
			rightMiddle : function() {
				x = this._rightCommon();
				y = this._topCommon()
						- (self._calendarMainNode.outerHeight() / 2)
						+ self.dateInputNode.outerHeight() / 2
			},
			...
			bottomMiddle : function() {
				y = this._bottomCommon();
				x = this._leftCommon() + (dynamicsGeometry.width / 2)
						+ self._calendarMainNode.width() / 2;
			},
			bottomLeft : function() {
				y = this._bottomCommon();
			...
			_topAutoCommon : function() {
				var y;
				if (+(this._topCommon() + self._calendarMainNode.outerHeight() - $(
						window).scrollTop()) > $(window).height()) {
					y = ($(window).height() + $(window).scrollTop())
							- self._calendarMainNode.outerHeight();
					if ($(window).height() < self._calendarMainNode
							.outerHeight())
						y = $(window).scrollTop();
				} else
					y = this._topCommon();
			...
			_leftAutoCommon : function() {
				var x;
				var offsetLeft = this._leftCommon()
						+ self._calendarMainNode.outerWidth();
				if (offsetLeft + self._calendarMainNode.outerWidth() > $(window)
						.width()) {
		...
		if (isNaN(x) && isNaN(y)) {
			...
			var lastFunctionWord = y.substr(0, 1).toUpperCase()
					+ this.view.position[1].substr(1);
			var func = (x === "*" && y === "*") ? "**"
					: y == "*" ? firstFunctionWord : firstFunctionWord
							+ lastFunctionWord;
	...
	_dateSelect : function(e) {
		...
		if (this.selectionRule.single) {
			this._selectedDateNode
					&& this
							._removeClassFromSelectedDate(this._selectedDateNode);
			...
			if (this.selectionRule.rangeDisableSelect
					|| !this._checkForRangeDisable(
							this._startRangeSelectedDateBk,
							this._endRangeSelectedDateBk,
							this._isFromDateOpened)) {
				optionToNotify = {
					fromDate : this._startRangeSelectedDate,
					toDate : this._endRangeSelectedDate,
					timeStamp : new Date().getTime()
				};
				if (this._isFromDateOpened) {
					this._startRangeSelectedNode
							&& this
									._removeClassFromSelectedDate(this._startRangeSelectedNode);
					this._startRangeSelectedNode = elem;
					this._startRangeSelectedDate = this._startRangeSelectedDateBk = dateObj;
					$.extend(optionToNotify, {
						fromDate : this._startRangeSelectedDate
					});
					this._notifySubscribers(this.events.select, optionToNotify);
					this.dateInputNode.val(date);
					this.show({
						currentTarget : this._dateInputToNode[0]
					});
				} else if (this._isToDateOpened) {
					this._endRangeSelectedNode
							&& this
									._removeClassFromSelectedDate(this._endRangeSelectedNode);
					...
					this._closeIfFill() && !this._startRangeSelectedDate
							&& this.show(this.dateInputNode);
				}
				...
				this._endRangeSelectedDateBk = this._endRangeSelectedDate ? new Date(
						this._endRangeSelectedDate.getTime())
						: null;
				this._startRangeSelectedDateBk = this._startRangeSelectedDate ? new Date(
						this._startRangeSelectedDate.getTime())
						: null;
			}
		}
	...
	_currentBriefDateSet : function(currentIndicator, type) {
		if (currentIndicator) {
			if (type === this._briefDateScope.briefDate)
				(currentIndicator || this._currentIndicator)["text"]
						(this.monthsFull[this.currentDate.getMonth()] + " "
				...
				var years = this._getYearsRange(this.currentDate.getFullYear());
				(currentIndicator || this._currentIndicator)["text"](years[0]
	...
	_updateDateMatricesExactDate : function(exactDate, currentIndicator,
			isInner) {
		this.currentDate = new Date(exactDate.getTime());
		this._updateDateMatricesCommon(isInner);
	},
	_updateDateMatricesCommon : function(isInner) {
		!isInner && this._notifySubscribers(this.events.change, {
			date : this.currentDate,
			string : this._dateFormatting(this.currentDate),
			timeStamp : new Date().getTime()
		});
		...
		if (this.view.display === this._displaySet.days) {
			...
			if (this.selectionRule.range) {
				if (this._isFromDateOpened)
					this._endRangeSelectedDate
							&& this._disableDates(this._endRangeSelectedDate,
									false);
				else if (this._isToDateOpened)
					this._startRangeSelectedDate
							&& this._disableDates(this._startRangeSelectedDate,
									true);
			}
			for ( var i in this._dateRows) {
				var objectDateInfo = this
						._bindDateToRows(+i, this._dateRows[i]);
				this._dateRows[i].data("click-behaviour",
						objectDateInfo.isNextMonth);
	...
	_markToday : function(date) {
		if (this.modules.today) {
			...
			this._todayNode
					&& this._todayNode.removeClass(
							this.className._inner.todayInGrid).removeClass(
							this.className.todayBox);
			for ( var i in this._dateRows) {
				if (this._dateEqualsTo(this._dateRows[i].data("date"), date)) {
					this._todayNode = this._dateRows[i];
					$(this._dateRows[i]).addClass(
							this.className._inner.todayInGrid).addClass(
							this.className.todayBox);
				}
			}
		}
	...
	_bindDateToRows : function(day, dom) {
		...
		if (day >= currentDateInfo.firstDay
				&& day < daysNumber.nowMonthDays + currentDateInfo.firstDay) {
			day = day - currentDateInfo.firstDay + 1;
			isNextMonth = "not-avail";
			if (date.getDate() == day - 1 && date.getHours() == 0
					&& currentDateInfo.month == 2) {// DayLightSaving
				date = new Date(currentDateInfo.year, currentDateInfo.month,
						currentDateInfo.date + 1, 2)
			} else
				date.setDate(day);
		} else if (day < currentDateInfo.firstDay) {
			var calPrevDay = daysNumber.prevMonthDays
					- (currentDateInfo.firstDay - day - 1);
			...
			var calNextDay = (day + 1)
					- (currentDateInfo.firstDay + daysNumber.nowMonthDays);
			date.setMonth(date.getMonth() + 1);
	...
	_numberOfDaysInMonth : function(date) {
		...
		return {
			prevMonthDays : this._getNumberOfDays(month - 1 < 0 ? 11
					: month - 1, month - 1 < 0 ? year - 1 : year),
			nowMonthDays : this._getNumberOfDays(month, year),
			nextMonthDays : this._getNumberOfDays(month + 1 > 11 ? 1 : month,
	...
	_dateFormatting : function(date) {
		var day = date.getDate(), month = date.getMonth() + 1, year = date
				.getFullYear();
		var isNM = this.dateFormat.format[0] == 'NM'
				|| this.dateFormat.format[1] == 'NM'
				|| this.dateFormat.format[2] == 'NM';
		var dateMap = {
		...
		this.forEach(this.dateFormat.format, function(index, elem) {
			dateMap[elem]
					&& (dateString += dateMap[elem]
							+ (index < 2 ? dateSeparator : ""));
		}, this);
		return dateString;
	},
	...
	_disableDates : function(compareDate, fromOrTo) {
		...
		for ( var i in this._dateRows) {
			...
			if (this[fromOrTo ? "_isLessThan" : "_isBiggerThan"](dateInfo,
					rule, true)) {
	...
	_addRule : function(from, to, fromDate) {
		...
		if (rule) {
			...
			if (fromDate) {
				...
				this._lastIndexOfFromDate === 0
						&& (this._lastIndexOfFromDate = 1);
	...
	_interpretRule : function(from, to) {
		...
		if (fromIsDate && toIsDate) {
			...
			fromDateInfo = fromIsDate ? this._currentDateExpose(new Date(from))
					: this._giveMeRules(from);
			toDateInfo = toIsDate ? this._currentDateExpose(new Date(to))
					: this._giveMeRules(to);
		}
	...
	_giveMeRules : function(rule) {
		...
		typeof rule === "string"
				&& (rules.date = rules.month = rules.year = rule);
		return rules
	},
	...
	_isMatchToStringRule : function(dateInfo, rule) {
		...
		var dateMapRuleFuntion = {
			">" : this._isBiggerThan,
			"<" : this._isLessThan,
			"*" : function(infoDate, ruleSet) { // Factorial state for complex
				// query implementaion
				if (self._isBiggerThan(dateInfo, rule, true)
						|| rule.fromYear === "*")
					if (ruleSet.toYear === "*") {
						if (ruleSet.toMonth === "*") {
							if (ruleSet.toDate === "*")
						...
						if (ruleSet.toMonth === "*") {
							...
							if (ruleSet.toDate === "*") {
								return infoDate.year == ruleSet.toYear
										&& infoDate.month == ruleSet.toMonth;
							} else
								return infoDate.year == ruleSet.toYear
										&& infoDate.month == ruleSet.toMonth
										&& infoDate.date == ruleSet.toDate
	...
	_isMatchingToDisableRule : function(date) {
		...
		for ( var i in this._disableDateMap) {
			...
			rule
					&& (disabled = this._isInRuleCharset(rule) ? this
							._isMatchToStringRule(dateInfo, rule) : this
							._isBiggerThan(dateInfo, rule, true)
							&& this._isLessThan(dateInfo, rule, false));
			if (disabled)
				return disabled;
		}
		return disabled;
	...
	_checkForRangeDisable : function(from, to, isFromDateOpened) {
		if (from && to) {
			...
			isFromDateOpened ? toBackUp.setDate(toBackUp.getDate() - 1)
					: fromBackUp.setDate(fromBackUp.getDate() + 1);
	...
	_isBiggerThan : function(dateInfo, rule, isFrom) {
		...
		if (dateInfo.year > rule[helper + "Year"])
			return true;
		if (dateInfo.year == rule[helper + "Year"]
				&& dateInfo.month > rule[helper + "Month"])
			return true;
		return dateInfo.date >= rule[helper + "Date"]
				&& rule[helper + "Year"] == dateInfo.year
				&& rule[helper + "Month"] == dateInfo.month;
	...
	_domGenerator : function(type, attr, parent, textOrVal, prepend) {
		...
		return elem.attr(attr)[elem[0].value ? "val" : "text"](textOrVal)[parent ? (prepend ? "prependTo"
				: "appendTo")
				: "andSelf"](parent ? parent : undefined);
	},
	_addClass : function() {
		var classes = "";
		...
		this.forEach(arguments, function(index, item) {
			classes += item
					+ (index !== _addClassArgs.length - 1
							&& _addClassArgs.length !== 1 ? " " : "");
		}, this);
		return classes;
	},
	_dynamicsGeometry : function(currentTarget) {
		...
		var offsetTopOfTarget = $(currentTarget)[this.view.alwaysVisible ? "position"
				: "offset"]().top;
		var offsetLeftOfTarget = $(currentTarget)[this.view.alwaysVisible ? "position"
				: "offset"]().left;
	...
	_getDecisionOnEvents : function() {
		...
		this.on(this.events.hide, function(data) {
			(self._selectedDate || self._startRangeSelectedDate)
					&& self._updateDateMatricesExactDate(self._selectedDate
							|| self._startRangeSelectedDate, true);
	...
	_rangeStatesManager : function(e) {
		...
		if (e && e.currentTarget === this._dateInputToNode[0]) {
			...
			this._resetRules(false);
			this._startRangeSelectedDate
					&& this._addRule(this._startRangeSelectedDate, "<", true);
		} else {
			delete this._disableDateMap[this._lastIndexOfFromDate];
			this._endRangeSelectedDate
					&& this._addRule(this._endRangeSelectedDate, ">", false);
		}
		this._backToNormalDisableRule();
		this
				._positionThisNodePlease(e && this.selectionRule.range
						&& e.currentTarget === this._dateInputToNode[0] ? this._dateInputToNode[0]
						: this.dateInputNode);
	},
	...
	_ExpandOrCollapse : function(increase, date) {
		...
		var displayMap = {
			1 : function() {
				self._currentBriefDateSet(self._currentIndicator,
						self._briefDateScope.briefDate);
				self.view.display = self._displaySet.days;
			},
			2 : function() {
				self._currentBriefDateSet(self._currentIndicator,
						self._briefDateScope.yearName);
				self.view.display = self._displaySet.months;
			},
			3 : function() {
				self._currentBriefDateSet(self._currentIndicator,
						self._briefDateScope.yearsRange);
				self.view.display = self._displaySet.years;
			}
		};
		...
		var stateFunc = increase ? displayMap[levelMap[this.view.display] + 1]
				: displayMap[levelMap[this.view.display] - 1];
		if (stateFunc) {
			this.currentDate = date ? new Date(date.getTime())
					: this.currentDate;
			stateFunc();
	...
	_dateEqualsTo : function(compare, compareTo) {
		return compare.getFullYear() === compareTo.getFullYear()
				&& compare.getMonth() === compareTo.getMonth()
				&& compare.getDate() === compareTo.getDate();
	},
	...
	_addClassToSelectedDate : function(elem) {
		elem.addClass(this.className.selected + " "
				+ this.className._inner.selectedDate);
	},
	_removeClassFromSelectedDate : function(elem) {
		elem.removeClass(this.className.selected + " "
				+ this.className._inner.selectedDate);
	},
	_removeDisableClass : function(elem) {
		elem.removeClass(this.className.disabled + " "
				+ this.className._inner.disableDate);
	},
	_addDisableClass : function(elem) {
		elem.addClass(this.className.disabled + " "
				+ this.className._inner.disableDate);
	},
	_addClassToRange : function(from, to) {
		...
		for ( var i in this._dateRows) {
			...
			if (from && to) {
				elem.removeClass(this.className.betweenRange + " "
						+ this.className._inner.betweenDateNode);
				var dateExpose = this._currentDateExpose(date);
				if (this._isEqual(dateExpose, rule, true)
						|| this._isEqual(dateExpose, rule, false)) {
					this._addClassToSelectedDate(elem);
				} else if (this._isBiggerThan(dateExpose, rule, true)
						&& this._isLessThan(dateExpose, rule, false)) {
					elem.addClass(this.className.betweenRange + " "
							+ this.className._inner.betweenDateNode);
				}
			} else if (from) {
	...
	_removeClassFromRange : function() {
		for ( var i in this._dateRows)
			this._dateRows[i].removeClass(this.className.betweenRange + " "
					+ this.className._inner.betweenDateNode);
	...
	on : function(topic, callBack, scope) {
		if ((topic && typeof topic === "string") && callBack) {
			...
			this._topicMap[topic].push({
				func : callBack,
				scope : scope ? scope : this._calendarMainNode
			});
		}
	},
	show : function(e) {
		...
		this.selectionRule.range ? this._rangeStatesManager(e) : this
				._positionThisNodePlease(this.dateInputNode);
	},
	hide : function() {
	...
	reset : function() {
		if (this.selectionRule.range) {
			(this._startRangeSelectedNode || this._endRangeSelectedNode)
					&& this
							._removeClassFromSelectedDate(this[this._startRangeSelectedNode ? "_startRangeSelectedNode"
									: "_endRangeSelectedNode"]
									.add(this._endRangeSelectedNode));
			this._resetRules("BOTH");
			delete this._disableDateMap[this._lastIndexOfFromDate];
			delete this._disableDateMap[this._lastIndexOfToDate];
			this._endRangeSelectedDate = this._endRangeSelectedNode = this._startRangeSelectedDate = this._startRangeSelectedNode = this._lastIndexOfFromDate = this._lastIndexOfToDate = this._endRangeSelectedDateBk = this._startRangeSelectedDateBk = null;
	...
	setDate : function(date) {
		if (!isNaN(Date.parse(date)))
			this._updateDateMatricesExactDate(this._isDate(date) ? date
	...
	isDisable : function(date) {
		return !isNaN(Date.parse(date)) ? this
				._isMatchingToDisableRule(new Date(Date.parse(date))) : date;
	},
...
initializeBitCal = function() {
	...
	$(elems).each(
			function() {
				...
				$.extend(true, options, _interpretExtraOptions(elem),
						_interpretPosition(elem), _interpretDisableRules(elem),
						_interpretRange(elem), _interpretDateFormat(elem),
...
_interpretRange = function(elem) {
	...
	return {
		selectionRule : {
			range : range,
			rangeDisableSelect : canSelectDisableDate,
			single : !range
		}
	};
};
_interpretDateFormat = function(elem) {
	...
	return {
		dateFormat : $.extend({
			format : _parseJsonEngine(dateFormatArray, [])
		}, separator ? _parseJsonEngine("{" + separator + "}", [ "separator" ])
				: {
					separator : "-"
				})
	};
...
_parseJsonEngine = function(item, words) {
	...
	return typeof item === "string" ? JSON.parse(item.replace(/'/g, '"'))
			: item;
