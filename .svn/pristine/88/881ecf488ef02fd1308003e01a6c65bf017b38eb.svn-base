...
public class Url extends Bean implements Externalizable, Comparable<Url> {
	...
	public static String id(String url) {
		if (url != null) {
			...
			if (len > 512) {
				...
				while (i < len) {
					sb.append(".")
							.append(url.substring(i, Math.min(len, i + 512))
									.hashCode());
					i += 512;
				}
	...
	public void insert() {
		if (isMongo()) {
			...
			v.put("hashcode", url.hashCode());
			v.put("url", url);

			if (referer != null) {
				v.put("referer", referer);
			}
			...
			v.put("url_date", url_date - (numofparent * X.ADAY));

			v.put("web_date", web_date);

			v.put("numofknown", numofknown);

			v.put("numofparent", numofparent);
	...
	public void update() {
		...
		if (!v.isEmpty()) {
			update(tableName, v, "hashcode=? and url=?",
					new Object[] { url.hashCode(), url }, null);
		}
	}
	...
	public boolean exists2(Connection c) {
		if (isMongo()) {
			...
			// log.error(e1);
			// }
			Bean b = Bean.load(tableName, "hashcode=? and url=?", new Object[] {
					url.hashCode(), url }, Url.class);
	...
	public static Url load(String domain, String url) {
		...
		try {
			...
			stat = c.prepareStatement("select * from " + table(domain)
					+ " where hashcode=? and url=?");
	...
	private static List<Url> _eldestForUrl(String domain, int num) {
		...
		synchronized (lock(domain)) {
			...
			try {
				c = Bean.getConnection();
				stat = c.prepareStatement("select * from " + tableName
						+ " order by url_date limit ?");
				stat.setInt(1, num);
				r = stat.executeQuery();
				while (r.next()) {
					Url u = fromSet(r);
	...
	private static void updateUrlDate(List<Url> list) {
		...
		if (isMongo()) {
			...
			for (Url u : list) {
				 ...
				 * update url_date
				 */
				BasicDBObject today = new BasicDBObject("$set",
						new BasicDBObject().append("url_date", u.url_date));
			...
			try {
				...
				c = Bean.getConnection();
				stat = c.prepareStatement("update " + u.tableName
						+ " set url_date=? where hashcode=? and url=?");
				for (Url u1 : list) {
	...
	private static List<Url> _mongo_eldest(String domain, int num) {
		...
		try {
			BasicDBObject today = new BasicDBObject("$set",
					new BasicDBObject().append("url_date",
			...
			while (c.hasNext() && num-- > 0) {
				 ...
				 */
				collection.update(
						new BasicDBObject().append("_id", o.get("_id")), today);
	...
	private static void _insert(List<Url> urls) {
		...
		try {
			...
			while (urls.size() > 0) {
				...
				try {
					stat = c.prepareStatement("insert into "
							+ u.tableName
							+ "(hashcode, url, depth, url_date, referer, cookie, type, numofparent, create_date) values(?,?,?,?,?,?,?,?,?)");
